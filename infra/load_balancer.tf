...
#
# Dress rehearsals use run-scoped names for disposable load-balancer
# scaffolding. Managed SSL certificates stay production-only because they
# validate real public domains and cannot be meaningfully tested without
# owning a rehearsal DNS name.

locals {
  lb_name_prefix = var.is_dress_rehearsal ? "blog-${local.rehearsal_suffix}" : "blog"
}
...
resource "google_compute_managed_ssl_certificate" "blog_ssl" {
  ...
  project = var.project_id
  name    = "${local.lb_name_prefix}-ssl-cert"
...
resource "google_compute_target_https_proxy" "blog_https_proxy" {
  ...
  ssl_certificates = [google_compute_managed_ssl_certificate.blog_ssl[count.index].id]
...
resource "google_compute_global_forwarding_rule" "blog_https_forwarding_rule" {
  ...
  target     = google_compute_target_https_proxy.blog_https_proxy[count.index].id
  port_range = "443"
